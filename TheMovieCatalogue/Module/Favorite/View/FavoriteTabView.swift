                    ...
                    VStack(spacing: 0) {
                        ...
                        TabView(selection: $selectedTab,
                            ...
                            if presenter.movieList.filter{item in item.isFavorite}.count < 1 {
                                ...
                                FavoriteLayout(presenter: presenter, tvData: presenter.tvList, movieData: presenter.movieList, isMovie: true)
                                    .tag(0)
                            }
                            if presenter.tvList.filter{item in item.isFavorite}.count < 1 {
                                ...
                                FavoriteLayout(presenter: presenter, tvData: presenter.tvList, movieData: presenter.movieList, isMovie: false)
                                    .tag(1)
                            }
                        })
