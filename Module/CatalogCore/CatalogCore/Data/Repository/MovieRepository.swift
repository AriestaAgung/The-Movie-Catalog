...
extension MovieRepository: MovieRepositoryProtocol {
    public func addMovieFavorite(item: MovieListEntity) -> Observable<Bool> {
        return self.locale.addMovieFavoriteDetail(item: item)
    }
    
    public func getFavoriteMovieList() -> RxSwift.Observable<[MovieListModel]> {
        return self.locale.getFavoriteMovieList()
            .map{HomeMovieMapper.mapEntityToDomain(entities: $0).filter{$0.isFavorite == true}}
            .filter{!$0.isEmpty}
    }
    
    public func getFavoriteMovieDetail(id: Int) -> RxSwift.Observable<MovieListModel> {
    ...
    public func getMovieList() -> Observable<[MovieListModel]> {
